...
import com.ffr.booklibrary.circulation.core.model.exceptions.BookCannotBeRenewedException;
...
public class BookIssue extends BaseEntity {
    ...
    public boolean hasElapsed(final Instant currentTime) {
        return expirationTime.plus(renewals.stream()
                .map(BookRenewal::renewalDuration)
                .reduce(Duration.ofDays(0), (Duration a, Duration b) -> a.plus(b)))
                .isAfter(currentTime);
    }
    ...
    public static BookIssue createIssue(final UserId userId, final Instant expirationTime) {
        return BookIssue.builder().userId(userId).maximumRenewals(0).expirationTime(expirationTime).build();
    }
}
